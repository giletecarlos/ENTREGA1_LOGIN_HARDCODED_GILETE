...
<div class="cont">
  ...
  <form class="login-form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST" onsubmit="return validarForm()">
      ...
      <input type="text" class="user" placeholder="Usuario" name="email" id="email"/>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($email == null || $password == null) {
        ...
        for ($i = 0; $i < sizeof(EMAILS); $i++) {
            if ($email == EMAILS[$i] && password_verify($password, password_hash(CONTRASEÑAS[$i], PASSWORD_DEFAULT))) {
